...
MoYu FlowSub 是一款基于七牛云的 AI 实时双语同传字幕助手，面向外语演讲、技术分享、国际会议和网课场景。当前 Demo 已跑通前后端工程、会话管理、WebSocket 实时通信、麦克风音频采集、七牛云智能语音/FunASR/Mock ASR 降级链路、DeepSeek 实时翻译、上下文修正、字幕展示和延迟指标。
...
- 云能力：七牛云 Kodo 配置占位、七牛云智能语音真实识别、DeepSeek-V4-Pro 真实翻译
...
15. 点击“结束会话”，会话状态变为 `FINISHED`，WebSocket 断开。
16. 访问 `http://localhost:8080/api/qiniu/status` 返回七牛云配置占位状态。
